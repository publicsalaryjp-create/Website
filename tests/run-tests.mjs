...
test("calculateOvertimeAllowance: 「単価×割増率」を区分ごとに四捨五入してから時間数を掛けて合算する", () => {
  // 端数のある時間単価で、「単価×割増率」を先に四捨五入してから時間数を掛ける方式であることを確認する
  const r = context.calculateOvertimeAllowance(1399.7, {
    weekdayNormalHours: 3,
    weekdayNightHours: 0,
    holidayNormalHours: 1,
    holidayNightHours: 0,
  });
  // 平日: round(1399.7*1.25)=round(1749.625)=1750 -> 1750*3=5250
  // 休日: round(1399.7*1.35)=round(1889.595)=1890 -> 1890*1=1890
  // 合算: 5250+1890=7140
  // （先に時間数まで掛けてから四捨五入する方式だと round(1399.7*1.25*3)=round(5248.875)=5249 になり結果が異なる）
...
test("calculateSalary: 扶養手当とそれに対応する地域手当は賞与の算定基礎に含まれない", () => {
  const baseInput = {
    ...
    teishuMonthsJune: 1.2625,
    teishuMonthsDecember: 1.2625,
    bonusRoleStageAdditionRate: 0.05,
    meritRateJune: 1.0225,
    meritRateDecember: 1.0225,
  };
  const withoutDependent = context.calculateSalary(baseInput);
  const withDependent = context.calculateSalary({ ...baseInput, parentCount: 1 });
...
test("calculateSalary: 一般職員の成績区分ごとに勤勉手当が変わる（下限値を採用）", () => {
  ...
  // kinbenJune = floor(100000 * meritRate)
