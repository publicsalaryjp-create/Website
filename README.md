...
- 扶養手当（令和8年4月現在。子は15歳以下/16〜22歳で額が異なる、父母等。配偶者手当は廃止済みのため対象外。
  人数はボタン選択式で入力）
- 住居手当（「支給の有無」を選択。支給あり＝借家など、を選んだ場合のみ月額を直接入力。
  支給なし＝持ち家などは0円。持家世帯向け住居手当は制度上廃止済み）
- 本府省業務調整手当（本省手当。本府省勤務者向け。「支給の有無」のみを選択し、金額は
  選択中の職務の級から自動計算。既定値は支給なし＝0円。級別の参考額を折りたたみ表で表示）
