...
**俸給表（`js/data.js` の `SALARY_TABLE`）は、正式な人事院官報別表を取得できなかったため、
実際の俸給表のカーブ（級・号俸が上がるほど昇給額が逓減する形）を模した参考値（推定値）です。
正確な金額ではありません。**

正式なデータをお持ちの場合は `data/salary-table.json` を配置してください。存在すればアプリが自動的にそちらを読み込み、
参考値の代わりに使用します（フォーマットは `data/README.md` 参照）。
...
index.html          画面
css/style.css        スタイル
js/data.js            俸給表・手当データ、公式データの読み込み処理
js/calculator.js      計算ロジック（純粋関数）
js/app.js             DOM配線
data/                 公式俸給表データの配置場所（任意）
```
